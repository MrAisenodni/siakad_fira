...
class MidtermController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $check = $this->exams->whereRaw("date LIKE '%".date('Y-m-d', strtotime(str_replace('/', '-', $input['date'])))."%' AND disabled = 0 AND lesson_id = ".$input['lesson']." AND type = 'uts' AND class_id = ".$input['clazz']." AND ".$clock_in." BETWEEN TIME_TO_SEC(clock_in) AND TIME_TO_SEC(clock_out)")->first();
